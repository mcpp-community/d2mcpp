...
// license: Apache-2.0
// file: src/en/cpp11/tests/17-pod-type/2.cpp
//
...
extern "C" void c_send_header(const void* data, std::size_t size) {
    ...
    d2x::check_eq(size, sizeof(MessageHeader), "size == sizeof(MessageHeader)");
...
int main() {
    ...
    d2x::check_eq(g_last_header.type, msg.type, "g_last_header.type == msg.type");
    d2x::check_eq(g_last_header.len, static_cast<std::uint32_t>(msg.payload.size()), "g_last_header.len == static_cast<std::uint32_t>(msg.payload.size())");
    d2x::check_eq(g_last_header.flags, static_cast<std::uint16_t>(0), "g_last_header.flags == static_cast<std::uint16_t>(0)");
