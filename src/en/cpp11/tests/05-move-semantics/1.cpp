...
// license: Apache-2.0
// file: src/en/cpp11/tests/05-move-semantics/1.cpp
//
...
int main() { // No compiler optimization
 
    {
        ...
        d2x::check_eq(move_assignment_counter, 1, "move_assignment_counter == 1");

        Buffer buff2;

        buff2 = process(buff1); // Case 2: Intermediate object assignment

        d2x::check_eq(move_assignment_counter, 2, "move_assignment_counter == 2");

        buff2 = buff1; // Case 3: Explicit move assignment

        d2x::check_eq(move_assignment_counter, 3, "move_assignment_counter == 3");
