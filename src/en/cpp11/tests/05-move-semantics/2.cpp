...
// license: Apache-2.0
// file: src/en/cpp11/tests/05-move-semantics/2.cpp
//
...
int main() { // Move semantics - Demonstrating moving resources not objects
 
    {
        ...
        d2x::check(&b1 != &b2, "&b1 != &b2"); // b1 and b2 are different objects
        d2x::check(old_b1_data_ptr == b2.data_ptr(), "old_b1_data_ptr == b2.data_ptr()");
        d2x::check(b1.data_ptr() == nullptr, "b1.data_ptr() == nullptr"); // b1's resources have been moved
