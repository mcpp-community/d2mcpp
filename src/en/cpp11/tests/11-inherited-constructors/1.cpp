...
// license: Apache-2.0
// file: src/en/cpp11/tests/11-inherited-constructors/1.cpp
//
...
int main() { // Do not directly modify the code in the main function
    ...
    { // Basic test
        ...
        d2x::check(studentTest.age_valid() == true, "studentTest.age_valid() == true");
        d2x::check(studentTest.score_valid() == true, "studentTest.score_valid() == true");
        d2x::check(studentTest.to_string() == "{001, 张三, 18, 0}", "studentTest.to_string() == \"{001, 张三, 18, 0}\"");
    }
    ...
    { // Boundary test
        ...
        d2x::check(studentTest.score_valid() == true, "studentTest.score_valid() == true");
        d2x::check(studentTest.age_valid() == false, "studentTest.age_valid() == false");

        studentTest.set_score(101);
        studentTest.set_age(200);
        d2x::check(studentTest.score_valid() == false, "studentTest.score_valid() == false");
        d2x::check(studentTest.age_valid() == true, "studentTest.age_valid() == true");
