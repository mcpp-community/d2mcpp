...
```
mcpp.toml                 workspace = [harness, intro, cpp11, cpp14, en/*, d2x/buildtools/mcpp]
harness/                  练习脚手架包(纯模块 d2x.harness,零宏)
intro/tests/hello-mcpp.cpp
cpp11/tests/<NN-topic>/<K>.cpp        49 题
cpp14/tests/00-generic-lambdas/{0,1}.cpp
en/{intro,cpp11,cpp14}/               en 镜像工程(lang=en 时启用,与 zh 互斥)
...
```
mcpp test -p cpp11              原生:进度表
mcpp test -p cpp11 00-auto      原生:单题(子串过滤)
...
| 退出码 | 首次调用注册 atexit：有失败断言或未拆 wait → `_Exit(1)`。裸 `mcpp test` 因此天然能判对错 |
